...
Tests motion detection/rejection enhancement in isolation
"""
import sys
import os
...
def process_pure_images(subject_path, subject_name, use_motion, fps=30, frame_skip=2):
    """Process PURE images with motion detection only"""
    ...
    # ITERATION 4c: Test motion detection ONLY
    processor = RPPGProcessor(
        ...
        use_motion_detection=use_motion  # TEST THIS
    )
...
def evaluate_configuration(dataset_root, use_motion, config_name):
    ...
    print(f"Motion Detection: {'ENABLED' if use_motion else 'DISABLED'}")
    print(f"{'='*70}")
    ...
    for i, subject_dir in enumerate(subject_dirs):
        ...
        try:
            ...
            pred_hr, pred_ts = process_pure_images(subject_dir, subject_name, use_motion, fps=30, frame_skip=2)
...
def generate_comparison_report(baseline_results, enhanced_results):
    ...
    report = []
    report.append("=" * 80)
    report.append("ITERATION 4c: MOTION DETECTION EVALUATION")
    report.append("=" * 80)
    report.append("")
    report.append("COMPARISON: Iteration 2 Baseline vs 4c (+ Motion Detection ONLY)")
    report.append("")
    report.append("-" * 80)
    ...
    report.append(f"{'Metric':<20} {'Baseline':<15} {'4c (Motion)':<15} {'Delta':<15} {'% Change':<10}")
    report.append("-" * 80)
    ...
    report.append("-" * 80)
    report.append(f"{'Subject':<12} {'Baseline MAE':<12} {'4c MAE':<12} {'Delta':<15} {'Status':<10}")
    report.append("-" * 80)
    ...
    if overall_improvement > 5:
        verdict = "✅ BENEFICIAL - Include motion detection"
    elif overall_improvement > 0:
        verdict = "⚠️ MARGINAL - Consider including"
    else:
        verdict = "❌ HARMFUL - Exclude motion detection"
...
def main():
    ...
    print("=" * 80)
    print("ITERATION 4c EVALUATION: Motion Detection ONLY")
    print("=" * 80)

    # Baseline
    print("\nPhase 1: Evaluating Baseline (No Enhancements)...")
    baseline_results = evaluate_configuration(dataset_root, use_motion=False, config_name="Baseline")

    # With motion detection
    print("\nPhase 2: Evaluating with Motion Detection...")
    enhanced_results = evaluate_configuration(dataset_root, use_motion=True, config_name="4c + Motion")
    ...
    # Save results
    with open('iteration4c_comparison.txt', 'w', encoding='utf-8') as f:
        f.write(report)

    try:
        print("\n" + report)
    except UnicodeEncodeError:
        print("\nReport generated (see iteration4c_comparison.txt for full details)")
    ...
    df_enhanced['configuration'] = 'Iteration4c_Motion'

    combined = pd.concat([df_baseline, df_enhanced])
    combined.to_csv(f'iteration4c_results_{timestamp}.csv', index=False)

    print(f"\n{'='*80}")
    print("Iteration 4c evaluation complete!")
    print(f"Comparison report: iteration4c_comparison.txt")
    print(f"Detailed results: iteration4c_results_{timestamp}.csv")
    print(f"{'='*80}")
