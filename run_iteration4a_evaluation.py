...
Tests illumination normalization enhancement in isolation
"""
import sys
import os
...
def process_pure_images(subject_path, subject_name, use_illumination, fps=30, frame_skip=2):
    """Process PURE images with illumination normalization only"""
    ...
    # ITERATION 4a: Test illumination normalization ONLY
    processor = RPPGProcessor(
        fps=fps//frame_skip,
        use_multi_roi=True,
        use_mediapipe=False,
        use_illumination_norm=use_illumination,  # TEST THIS
        use_temporal_filter=False,  # DISABLED
...
def evaluate_configuration(dataset_root, use_illumination, config_name):
    ...
    print(f"Illumination Normalization: {'ENABLED' if use_illumination else 'DISABLED'}")
    print(f"{'='*70}")
    ...
    for i, subject_dir in enumerate(subject_dirs):
        ...
        try:
            ...
            pred_hr, pred_ts = process_pure_images(subject_dir, subject_name, use_illumination, fps=30, frame_skip=2)
...
def generate_comparison_report(baseline_results, enhanced_results):
    ...
    report = []
    report.append("=" * 80)
    report.append("ITERATION 4a: ILLUMINATION NORMALIZATION EVALUATION")
    report.append("=" * 80)
    report.append("")
    report.append("COMPARISON: Iteration 2 Baseline vs 4a (+ Illumination Normalization ONLY)")
    report.append("")
    report.append("-" * 80)
    ...
    report.append(f"{'Metric':<20} {'Baseline':<15} {'4a (Illumin)':<15} {'Delta':<15} {'% Change':<10}")
    report.append("-" * 80)
    ...
    report.append("-" * 80)
    report.append(f"{'Subject':<12} {'Baseline MAE':<12} {'4a MAE':<12} {'Delta':<15} {'Status':<10}")
    report.append("-" * 80)
    ...
    if overall_improvement > 5:
        verdict = "✅ BENEFICIAL - Include illumination normalization"
    elif overall_improvement > 0:
        verdict = "⚠️ MARGINAL - Consider including"
    else:
        verdict = "❌ HARMFUL - Exclude illumination normalization"
...
def main():
    ...
    print("=" * 80)
    print("ITERATION 4a EVALUATION: Illumination Normalization ONLY")
    print("=" * 80)

    # Baseline
    print("\nPhase 1: Evaluating Baseline (No Enhancements)...")
    baseline_results = evaluate_configuration(dataset_root, use_illumination=False, config_name="Baseline")

    # With illumination normalization
    print("\nPhase 2: Evaluating with Illumination Normalization...")
    enhanced_results = evaluate_configuration(dataset_root, use_illumination=True, config_name="4a + Illumination")
    ...
    # Save results
    with open('iteration4a_comparison.txt', 'w', encoding='utf-8') as f:
        f.write(report)

    try:
        print("\n" + report)
    except UnicodeEncodeError:
        print("\nReport generated (see iteration4a_comparison.txt for full details)")
    ...
    df_enhanced['configuration'] = 'Iteration4a_Illumination'

    combined = pd.concat([df_baseline, df_enhanced])
    combined.to_csv(f'iteration4a_results_{timestamp}.csv', index=False)

    print(f"\n{'='*80}")
    print("Iteration 4a evaluation complete!")
    print(f"Comparison report: iteration4a_comparison.txt")
    print(f"Detailed results: iteration4a_results_{timestamp}.csv")
    print(f"{'='*80}")
